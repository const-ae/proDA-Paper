...
run_perseus <- function(data, tag="", return_null_if_missing = FALSE) {
  
  hash <- digest::digest(data)  
  prefix_in <- "tmp/de_graaf/perseus/input"
  prefix_out <- "tmp/de_graaf/perseus/output"
  ...
  if(!file.exists(file.path(prefix_out, result_file_name))){
    ...
    read_tsv(paste0(prefix_out, "/", result_file_name), 
             comment = "#") %>%
      mutate(pval = as.numeric(str_replace(`Student's T-test q-value Syn_Cond1_Syn_Cond2`, ",", "."))) %>%
      mutate(adj_pval = as.numeric(str_replace(`Student's T-test q-value Syn_Cond1_Syn_Cond2`, ",", "."))) %>%
      transmute(name=Name, pval, adj_pval, diff=0) 
  }else{
    read_tsv(paste0(prefix_out, "/", result_file_name), 
             comment = "#") %>%
      mutate(pval = as.numeric(str_replace(`Student's T-test q-value Syn_Cond1_Syn_Cond2`, ",", "."))) %>%
